...
- **Presentation Decoupling & UI-Agnostic Domain Services:** Backend domain services (`internal/*` excluding `internal/api/http`) MUST NEVER return HTML markup (`<b>`, `<br>`, etc.) or layout-specific formatting strings. All rendering, dialogue formatting, and presentation styling belong strictly to the presentation/client layer. Domain service results MUST return pure, structured data (models, counts, facts, or boolean flags like `SentToDepot`). HTTP handlers format presentation messages using standardized envelopes (`writeSuccess` and `writeAppError`).
  - **Continuous Mechanical Verification**: Enforced automatically via Go AST static analysis (`internal/architecture/presentation_lint_test.go`) during `make check` and CI.
