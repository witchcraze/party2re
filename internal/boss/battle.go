...
func (s *Service) StartSealingBattle(ctx context.Context, partyID, leaderCharID string) (SealingBattleResult, error) {
	...
	err := s.runInTx(ctx, func(txCtx context.Context) error {
		...
		if len(members) > stage.MaxMembers {
			return ErrPartyNotReady
		}
		...
		if isWin {
			...
			// 10b. Elapsed time crystal reward with clear-time multiplier
			elapsed := time.Since(p.CreatedAt)
			if elapsed < 0 {
				elapsed = 0
			}
			rewardCrystals = calculateTotalCrystals(stage, orderedChars, elapsed)
			...
			for _, m := range members {
				...
				if rewardCrystals > 0 {
					if err := c.AddCrystal(rewardCrystals); err != nil {
