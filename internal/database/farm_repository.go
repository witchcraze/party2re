...
func (r *FarmRepository) PlantSeed(ctx context.Context, characterID string, plotIndex int, seedType farm.SeedType, seedDef farm.SeedDefinition, now time.Time) (farm.FarmPlot, error) {
	...
	err := RunInTx(ctx, r.db, func(txCtx context.Context) error {
		...
		if errors.Is(err, sql.ErrNoRows) || seedQuantity <= 0 {
			return farm.ErrInsufficientSeed
		}
		if err != nil {
			return err
		}
		...
		if seedQuantity == 1 {
			if _, err := executor.ExecContext(txCtx, "DELETE FROM inventory_items WHERE id = ?", seedItemID); err != nil {
				return err
			}
		} else {
			if _, err := executor.ExecContext(txCtx, "UPDATE inventory_items SET quantity = quantity - 1 WHERE id = ?", seedItemID); err != nil {
				return err
			}
		}
...
func (r *FarmRepository) WaterPlot(ctx context.Context, characterID string, plotIndex int, now time.Time) (farm.FarmPlot, error) {
	...
	err := RunInTx(ctx, r.db, func(txCtx context.Context) error {
		...
		if p.ComputeCurrentStatus(now) != farm.PlotStatusGrowing {
			return farm.ErrPlotNotGrowing
		}
		if p.Watered {
			return farm.ErrAlreadyWatered
		}
...
func (r *FarmRepository) FertilizePlot(ctx context.Context, characterID string, plotIndex int, now time.Time) (farm.FarmPlot, error) {
	...
	err := RunInTx(ctx, r.db, func(txCtx context.Context) error {
		...
		if p.Fertilized {
			return farm.ErrAlreadyFertilized
		}
		...
		if errors.Is(err, sql.ErrNoRows) || fertQuantity <= 0 {
			return farm.ErrInsufficientFertilizer
		}
		if err != nil {
			return err
		}
		...
		if fertQuantity == 1 {
			if _, err := executor.ExecContext(txCtx, "DELETE FROM inventory_items WHERE id = ?", fertItemID); err != nil {
				return err
			}
		} else {
			if _, err := executor.ExecContext(txCtx, "UPDATE inventory_items SET quantity = quantity - 1 WHERE id = ?", fertItemID); err != nil {
				return err
			}
		}
...
func (r *FarmRepository) HarvestPlot(ctx context.Context, characterID string, plotIndex int, rewardGold int, rewardItemID string, yield int, now time.Time) (farm.FarmPlot, error) {
	...
	err := RunInTx(ctx, r.db, func(txCtx context.Context) error {
		...
		if status == farm.PlotStatusWithered {
			return farm.ErrPlotWithered
		}
		if status != farm.PlotStatusMature {
			return farm.ErrPlotNotMature
		}
