...
for i in {1..30}; do
    if docker compose exec -T mariadb mariadb -h 127.0.0.1 -u root -proot -e "SELECT 1" >/dev/null 2>&1; then
        break
    fi
    sleep 0.5
done

docker compose exec -T mariadb mariadb -h 127.0.0.1 -u root -proot -e \
