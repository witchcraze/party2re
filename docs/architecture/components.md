...
| **Ranking** | `internal/ranking` | 16 leaderboards (including authentic 英雄 kill_m, 魔王 mao_c, 勇者 hero_c), Hall of Fame (legend.cgi), weekly job change ranking with Sunday rotation, Valkey caching, singleflight stampede guard | Character, Player, Valkey, Scheduling | Valkey `party2:ranking:snapshot:*`; MariaDB `ranking_snapshots`, `legend_records`, `weekly_job_changes` |
