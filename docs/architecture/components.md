...
- **Casino** (`internal/casino`):
  ...
  - **Persistence:** `casino_accounts` in `internal/database/casino_repository.go` (financial SSOT) with deterministic lock hierarchy (Rank 0 Shared Peer Entity -> Rank 2 Character -> Rank 5 Depot -> Rank 8 Casino Account). Candidate C architecture implemented in Issue #635 migrating ephemeral multiplayer rooms and turn state (`casino_rooms`, `casino_room_members`) to Valkey Master (`party2:casino:*`, `ValkeyRoomRepository`) with 1800s sliding TTL, active ZSet index, and Two-Phase Settlement. Legacy tables preserved pending follow-up schema cleanup migration.
