...
- **King & World Boss Battles** (`internal/boss`):
  - **Responsibility:** Legendary raid/boss encounters across 10 progressive tiers and ultimate world boss tier, entry gate requirements (level gate, prerequisite tier completion, daily attempt limits), milestone first-clear bonuses, item drop rewards, and boss clear leaderboard. Supports real-time shared World Boss encounters via Valkey Master atomic Lua scripting (`boss_damage.lua`), `{boss:<boss_id>}` cluster hash tagging, deterministic killer election, and idempotent MariaDB Two-Phase Settlement (`RaidCoordinator`, `ValkeyRaidRepository`, SSOT: [`docs/architecture/transient-boss-hp.md`](transient-boss-hp.md)).
  - **Dependencies:** Core Battle Engine, Character repository, Inventory repository, Core Progression, Valkey Master (`party2:boss:{boss:<boss_id>}:*`).
  - **Persistence:** `character_boss_records` and `boss_challenge_history` tables in `internal/database/boss_repository.go` with atomic reward and record updates; volatile raid state in Valkey Master.
...
- **Cross-Domain Application Runtime Primitives & Transaction Runner (`internal/economy.TransactionRunner`)**: Reusable platform primitives (`economy.Service`, `ExecuteTransaction`, `economy.Run[T]`) abstracting wallet currency deductions/credits (`DeductGold`, `AddGold`, `TransferGold`, `DeductSmallMedals`, `AddSmallMedals`), inventory operations (`GrantItem`, `ConsumeItemInstance`, `ConsumeItemDefinition`), and atomic domain operations. Mechanically enforces the global deterministic lock order (`characters` Rank 2 -> `inventory_items` Rank 3 -> domain secondary tables Rank 8) and strict pre-condition balance checks (`balance >= cost`) with integer overflow prevention (`SafeMultiply`). Documented in [`cross-domain-primitives.md`](cross-domain-primitives.md).
