...
- **Guild** (`internal/guild`):
  - **Responsibility:** Guild founding (5,000G), membership lifecycle with formal application and approval gating (`参加申請中`, `あたえる` approval, `追放` rejection/expulsion with notification letters), dynamic Guild Points (`gpoint`) accrual across social/combat activities and server-wide rankings, broadcast member callouts (`よびかける`, +1 GP), custom member role titles (up to 6 full-width characters via `あたえる`), visual personalization (unique hex color, 3,000G guild mark, catalog-priced wallpapers), notice board, 20-day inactivity automatic disbandment (`guild_inactivity_check` scheduled action), and administrative controls (kick, leadership transfer, disband) (#490, #591).
...
- **Casino** (`internal/casino`):
  - **Responsibility:** Casino currency exchange (1 Coin = 20 G), account management, Multi-Player Room Lobby (2..8 capacity, speed 12/18/28s, 9 bet rates, password, spectator permissions, leader controls), authentic 13-card forehead Indian Poker with card masking and round escalations, 18 authentic prizes with atomic direct Depot delivery, Slot Machine (3-reel, 5-symbol paytable, 100x 777 jackpot), Doppelganger, and High & Low; fictional solo dealer poker completely purged (#486).
  - **Dependencies:** Character repository (wallet gold), Depot repository (`character_depots`).
  - **Persistence:** `casino_accounts`, `casino_rooms`, and `casino_members` tables in `internal/database/casino_repository.go` and `casino_room_repository.go` with deterministic lock hierarchy (Rank 0 Shared Peer Entity -> Rank 2 Character -> Rank 5 Depot -> Rank 8 Casino Account).
...
- **Guild versus Guild (GvG) Combat** (`internal/gvg`):
  ...
  - **Persistence:** Ephemeral room state, rosters, and scores in Valkey Master (`party2:gvg:room:<id>`, `party2:gvg:character:<id>`, `party2:gvg:rooms`) with 30m TTL; durable guild standings and 7 trophy tiers in MariaDB Master `gvg_standings` and team colors in `guilds.color`.
...
- **Candidate C: Party & Matchmaking Wait Lobbies (`party`, `matchmaking`)**: Implemented via Valkey Master (`party2:party:lobby:<id>`, `party2:party:lobbies`, `party2:party:character:<character_id>`, `party2:party:ready:<party_id>:<character_id>`) to decouple ephemeral wait states and 60-second ready checks from relational database tables ([Issue #368](https://github.com/witchcraze/party2re/issues/368)), with legacy `parties` and `party_members` tables officially dropped via Migration 052 ([Issue #380](https://github.com/witchcraze/party2re/issues/380)).
