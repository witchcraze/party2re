...
- **Casino** (`internal/casino`):
  ...
  - **Persistence:** `casino_accounts`, `casino_rooms`, and `casino_members` tables in `internal/database/casino_repository.go` and `casino_room_repository.go` with deterministic lock hierarchy (Rank 0 Shared Peer Entity -> Rank 2 Character -> Rank 5 Depot -> Rank 8 Casino Account). Candidate C architecture blueprint formalized in Issue #635 for migrating ephemeral multiplayer rooms (`casino_rooms`, `casino_room_members`) to Valkey Master (`party2:casino:*`) with 1800s sliding TTL and Two-Phase Settlement.
