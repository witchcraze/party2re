...
Job definitions are content data loaded and validated at startup from `internal/core/job/data/`. Exposes definitions through a lookup contract. `CharacterJob` encapsulates transitions (`ChangeTo`), mastery (`RecordMastery`, `Master`), prerequisite tree checks (`_is_need_job`), milestone counters (`MonsterKills`, `PvPWins`, `HeroCount`, `MaoCount`, `CasinoWins`, `JobLevel`), CMP tier rating (`c_mp_tier`), and retained mastery SP. Coordinates level-20 job change transactions, atomic equipped armor consumption for FireFighter (`job-84`), item possession and consumption validation, temporary mastered-job memory snapshots (`よびおこす`), and character job mastery catalog / completion percentage calculation (`job_master.cgi`).
...
| **Collection** | `internal/collection` | Illustrated monster defeat, item, weapon, and armor discovery encyclopedias, 100% completion milestones, Hall of Fame induction hooks | Character, Notification | MariaDB `character_monster_book`, `character_item_collection`, `character_collection_completions` |
...
| **Helper Quest** | `internal/helperquest` | Delivery quests, alchemy rewards, guild contribution | Character, Depot, Item, Guild, Monster Ranch (via adapter) | MariaDB `helper_quests` |
...
| **Ranking** | `internal/ranking` | 16 leaderboards (including authentic 英雄 kill_m, 魔王 mao_c, 勇者 hero_c), Hall of Fame (legend.cgi) with decoupled completion induction hooks, weekly job change ranking with Sunday rotation, Valkey caching, singleflight stampede guard | Character, Player, Valkey, Scheduling | Valkey `party2:ranking:snapshot:*`; MariaDB `ranking_snapshots`, `legend_records`, `weekly_job_changes` |
...
| **Shop** | `internal/shop` | Town equipment/item/accessory shops (Mira), 10×/1000× accessory pricing, 48 synthesis recipes, Item 180 guarantee, 50% markdown, depot auto-delivery | Catalogs, Character, Inventory, Depot | MariaDB `characters`, `inventory_items`, `depot_items` (Rank 2→3→5) |
