...
| **Store** | `internal/store` | Player shop construction (50kG/90d), barter listings, interiors, Oracle Shop costume rental, home wallpapers | Character, Depot, Item, Guild, Timer, Valkey | MariaDB `character_stores`, `store_sales`, `store_interiors`, `character_homes` (Rank 0→2→5); Valkey `party2:daily:costume:*` |
