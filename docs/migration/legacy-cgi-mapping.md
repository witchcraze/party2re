...
| Legacy Script | Authentic Role / Action | Go Domain Implementation | HTTP Handler & Migrations | Design Doc & OpenAPI | Status | Pitfalls / Parity Traps |
| :--- | :--- | :--- | :--- | :--- | :---: | :--- |
| `lib/vs_monster.cgi`, `lib/adventure.cgi`, `lib/adventure_record.cgi` | 冒険・階層探索 (Adventure) | `internal/adventure/` | `internal/api/http/adventure.go`<br/>`migrations/006_adventures.sql`<br/>`migrations/009_adventure_battle_rewards.sql`<br/>`migrations/037_adventure_chronicle.sql` | `docs/design/adventure.md`<br/>`docs/design/stages-and-monsters.md`<br/>`docs/api/paths/adventure.json` | Reconciling (#795) | Stage 26 四季のダンジョン復元、宝箱ドロッププール設定、死にタイマー削除、日次$m{once}制限・特技ボーナス連携 |
| `lib/vs_king.cgi`, `lib/_win_vs_king.cgi` | 封印の魔王 (Boss) | `internal/boss/` | `internal/api/http/combat.go`<br/>`migrations/025_boss_battles.sql` | `docs/design/boss.md`<br/>`docs/api/paths/boss.json` | Reconciling (#794) | King 1..10の6人枠・固有スキル復元、クリア時間クリスタル倍率・曜日オーブ追加。※王の証・世界樹の葉自動蘇生は原典非存在の誤認（疲労度+20で挑戦） |
| `lib/_battle.cgi` | コア戦闘エンジン | `internal/core/battle/` | Core Engine | `docs/design/battle.md` | Reconciling (#790) | DQ式ダメージ計算式(0.5*Atk - 0.3*Def)、素早さ比較による命中・回避・会心の一撃(防御無視直撃)の復元 |
| `lib/_skill.cgi` | スキル発動エンジン | `internal/core/skill/` | Core Engine | `docs/design/jobs-and-skills.md` | Reconciling (#791) | 6大状態異常（動封・禁呪・鎖縛・混乱・猛毒・劇毒）の復元、毒の行動直後処理および毎行動1/5自然治癒判定 |
...
| `lib/vs_dungeon.cgi`, `party2/dungeon.cgi` | ダンジョン探索 | `internal/dungeon/` | `internal/api/http/adventure.go`<br/>`migrations/026_dungeon_exploration.sql` | `docs/design/dungeon.md`<br/>`docs/api/paths/dungeon.json` | Reconciling (#795) | Dungeon 16の日次$m{once}制限、タイル定義(0,1,S,B,I,2)の整合 |
| `lib/vs_challenge.cgi`, `party2/challenge.cgi` | 連戦チャレンジ | `internal/challenge/` | `internal/api/http/combat.go`<br/>`migrations/028_endurance_challenge.sql` | `docs/design/challenge.md`<br/>`docs/api/paths/challenge.json` | Reconciling (#789) | 🚨 創作: 4段階ソーシャル風難易度(novice..abyss)・リタイア安全持ち帰り(Cashout)。<br/>正: 原典9ステージ(0最弱逆襲..8真最強王者)、+10%固定モンスター成長、10%宝箱階層トリガー |
| `lib/quest.cgi`, `party2/party.cgi` | パーティ共闘クエスト | `internal/party/` | `internal/api/http/party.go`<br/>`migrations/048_party_and_coop_quests.sql`<br/>`migrations/052_drop_parties_and_party_members.sql` | `docs/design/party-system.md`<br/>`docs/api/paths/party.json` | Reconciling (#793) | 人数相乗効果補正（2p=+10%, 3p=+20%, 4p=+30%）の獲得EXP/Goldへの実乗算反映バグ修正 |
| `lib/helper.cgi` | 手助けクエスト | `internal/helperquest/` | `internal/api/http/helper_rescue.go`<br/>`migrations/031_rescue_and_helper.sql` | `docs/design/rescue-and-helper.md`<br/>`docs/api/paths/helper.json` | Reconciling (#792) | 🚨 誤認: プレイヤー間救難掲示板。<br/>正: 何でも屋 @リッカ。預かり所(Depot)からの装備/アイテム納品、牧場(Farm)からのモンスター納品、報酬Depot直送 |
...
| Legacy Script | Authentic Role / Action | Go Domain Implementation | HTTP Handler & Migrations | Design Doc & OpenAPI | Status | Pitfalls / Parity Traps |
| :--- | :--- | :--- | :--- | :--- | :--- | :---: | :--- |
| `lib/guild.cgi`, `lib/join_guild.cgi`, `party2/guild_list.cgi` | ギルド拠点・運営 | `internal/guild/` | `internal/api/http/guild.go`<br/>`migrations/016_guilds.sql` | `docs/design/guild.md`<br/>`docs/api/paths/guild.json` | Compliant | 活動動的GP、役職命名（6文字以内）、HEXカラー設定、20日無稼働自動解散。 |
| `lib/photo.cgi`, `party2/contest.cgi`, `party2/screen_shot.cgi` | 写真館・コンテスト | `internal/contest/` | `internal/api/http/contest.go`<br/>`migrations/046_photo_contest_and_gallery.sql` | `docs/design/photo-contest.md`<br/>`docs/api/paths/contest.json` | Reconciling (#801) | 10日周期・5枚以上成立・1位投票者1メダル。定期自動精算ワーカー（wireContestSettlement）の登録漏れ修正 |
...
| `lib/takarakuzi.cgi` | 宝くじ (@クラゲ) | `internal/lottery/` | `internal/api/http/lottery.go`<br/>`migrations/081_takarakuji.sql` | `docs/design/lottery.md`<br/>`docs/api/paths/lottery.json` | Reconciling (#800) | 30,000G、サーバー20枚上限、10日周期(1,11,21日)。重複候補の重み付け確率維持バグ修正 |
| `lib/lot.cgi` | 福引 (@案内人) | `internal/lottery/` | `internal/api/http/lottery.go`<br/>`migrations/018_lottery.sql` | `docs/design/lottery.md`<br/>`docs/api/paths/lottery.json` | Reconciling (#800) | 3補助券通常・300補助券裏福引、曜日別景品。Rule 00 IP保護（走鳥の野菜・幻獣の実）への名称同期 |
| `lib/casino.cgi`, `party2/casino.cgi`, `lib/_casino.cgi` | カジノラウンジ | `internal/casino/` | `internal/api/http/casino.go`<br/>`migrations/017_casino.sql` | `docs/design/casino.md`<br/>`docs/api/paths/casino.json` | Reconciling (#799) | 8人共有ラウンジ、一方通行コイン購入(20G)、景品Depot直送。退席時疲労度加算、ドキュメントの逆換金表記是正 |
...
| `lib/casino_highlow.cgi` | ハイローゲーム | `internal/casino/` | `internal/api/http/casino.go` | `docs/design/casino.md`<br/>`docs/api/paths/casino.json` | Reconciling (#799) | 13枚カード・5ラウンド・ポット分配。勝利時カジノ勝利数 (`cas_c`) 加算修正 |
| `lib/casino_indian.cgi` | インディアンポーカー | `internal/casino/` | `internal/api/http/casino.go`<br/>`migrations/054_casino_poker_sessions.sql` | `docs/design/casino.md`<br/>`docs/api/paths/casino.json` | Reconciling (#799) | 額カード・5ラウンド・勝負・降り。勝利時カジノ勝利数 (`cas_c`) 加算修正 |
| `lib/casino_doppel.cgi` | ドッペルゲンガー | `internal/casino/` | `internal/api/http/casino.go` | `docs/design/casino.md`<br/>`docs/api/paths/casino.json` | Reconciling (#799) | 8絵柄マッチング・親総取り。勝利時カジノ勝利数 (`cas_c`) 加算修正 |
| `lib/collection.cgi`, `lib/_add_collection.cgi`, `lib/_add_monster_book.cgi`, `party2/view_monster.cgi` | 図鑑コレクション | `internal/collection/` | `internal/api/http/collection.go`<br/>`migrations/021_collection.sql` | `docs/design/collection.md`<br/>`docs/api/paths/collection.json` | Reconciling (#797) | 戦闘・ボス討伐勝利時のモンスター図鑑記録（RecordMonsterDefeat）配線、100%コンプリート時通知・伝説トリガー |
