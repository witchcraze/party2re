...
| `party2/ranking.cgi`, `week_ranking.cgi`, `job_ranking.cgi`, `legend.cgi` | サーバー各種ランキング・殿堂入り | `internal/ranking/` | `internal/api/http/ranking.go`<br/>`migrations/035_rankings_and_leaderboards.sql`<br/>`migrations/089_legend_and_week_ranking.sql` | `docs/design/ranking.md`<br/>`docs/api/paths/ranking.json`<br/>`docs/api/paths/legends.json` | Compliant (#798) | 殿堂入り（legend.cgi 6称号永久記録）、週間転職ランキング（week_ranking.cgi 日曜0時週次ローテ・スナップショット凍結）、勝負師(cas_c)・錬金(alc_c)ランキング復元 |
