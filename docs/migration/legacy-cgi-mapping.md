...
| `lib/accessory.cgi` | 装飾品屋 (@ミラ) | *(Unimplemented)* | *(Unimplemented)* | `docs/design/shops.md` | Reconciling | 🚨 完全脱落: `ShopTypeAccessory`未定義、10倍/1000倍価格未実装、全48種アクセサリー合成(`@acces`)および秘薬(Item 180)保証が完全脱落 |
...
| `lib/secret.cgi` | 秘密の店 (@ヒミツジ) | `internal/secretshop/` | `internal/api/http/secretshop.go` | `docs/design/shops.md`<br/>`docs/api/paths/secretshop.json` | Reconciling | 転職7回以上(`job_lv >= 7`)、原典8種3倍価格、満杯時Depot転送、ぱふぱふ会話のみ。手持ち直接購入時の図鑑登録連携が未実装 |
