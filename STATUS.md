...
Last updated: Issue #282 — [Chore] Test/Scheduling: Add unit tests for Worker and ValkeyRepository using mocks
...
- **Depot** (`internal/depot`): 倉庫（アイテム・ゴールド預入・引出）。横断的ランタイムプリミティブ `economy.TransactionRunner` / `ExecuteTransaction` への移行完了（Issue #445）。二重実行パス（トランザクション vs 非トランザクションフォールバック）を撤廃し単一の決定論的実行パスへ一本化。Rank 2 (`characters`) -> Rank 3 (`inventory_items`) -> Rank 5 (`character_depots`) の行ロック順序遵守とコードスリム化（315行、単体テストカバレッジ96.6%）を達成。
