...
Last updated: Issue #679 — [Refactor] Storage: Unify transactional reward item delivery and depot overflow routing into reusable domain helper
...
- **Depot** (`internal/depot`): ✅ Dynamic capacity (up to 500 slots), tiered expansion, sort, item sell, gold/item direct-send, standardized item consumption (`Consume`, `ConsumeOne`, `PurgeSlot`), enhancement-level & equipment stackability preservation (#558, #563), standardized `RefreshCapacity` helper across commerce modules (#559), and centralized transactional reward item delivery engine (`DeliverRewardItem`, `DeliverRewardItems`) enforcing Rank 3 -> Rank 5 lock ordering, dynamic capacity refresh, and configurable overflow policies (`PolicyTreatOverflowAsLost` vs `PolicyAbortOnDepotFull`) unified across `database`, `battle`, and `party` (#679).
...
- **Colosseum PvP (闘技場)** (`internal/pvp`): ✅ Real-time 2..8 player room recruitment (`quest.cgi:type=4`), Bet & Split prize pool mechanics, 9 team colors (`@ぱーてぃー`), multi-round party battle resolution (`_battle.cgi:486`), multi-team survivor determination (#594), durable `pvp_wins` (`$m{kill_p}`) tracking, draw refund on 10 rounds; Valkey room repository standardized to Candidate C keyspace (`party2:pvp:rooms:active`), `UpdatedAt` scoring, lazy ZSet pruning (#644), distributed room locking (`party2:pvp:lock:room:<room_id>`) with Lua script release and reentrant context execution preventing concurrent join over-admission (#652), and transactional settlement (`RunInTx`) with Rank 2 row locking (`FindByIDForUpdate` in ascending character ID order) and strict error propagation for bets, refunds, and prizes preventing silent currency loss (#661); fictional Elo rating arena purged (#481).
- **GvG Combat (ギルド戦)** (`internal/gvg`): ✅ Real-time 2..8 player guild battle rooms (`quest.cgi:type=5`, `vs_guild.cgi`), room GP prize pool seeding (2 GP initial + 1 GP per joiner), automatic guild color adoption, friendly guild battle prohibition (`color == '#FFFFFF'`), `@かいし` multi-guild validation and HP restoration, multi-round party battle resolution (`corebattle.ResolvePartyBattle`), multi-guild survivor determination (#594), round winner GP rewards (+3 GP), target wins (1-3 wins) match victory awards (prize pool GP + 1 Bronze Medal), all-participant +4 GP compensation, 10-round draw limit, and 7-tier victory medals & championship cups cascading promotion (5:1 ratios); Valkey room repository standardized to Candidate C keyspace (`party2:gvg:rooms:active`), `UpdatedAt` scoring, lazy ZSet pruning (#644), and distributed room locking (`party2:gvg:lock:room:<room_id>`) with Lua script release and reentrant context execution preventing concurrent join over-admission and prize pool race conditions (#652); fictional asynchronous Elo duels and match tables purged (#482).
- **Boss Battles (封印戦)** (`internal/boss`): ✅ 4-player Party Sealing Battles (`vs_king.cgi`, `stage/king1..10.cgi`, `king99.cgi`), Dejon banishment (+30% Tired), `@ふういん` resealing, HeroCount increment, celebration banquets, news broadcast; reward item delivery with `coreinventory.Inventory` capacity enforcement and depot overflow routing via `deliverRewardItems` (#657); fictional 1-day-3-attempts solo raid completely purged (#479).
- **Dungeon Exploration** (`internal/dungeon`): ✅ Grid-map exploration; Valkey Master run buffer (Lua CAS, 2h TTL, two-phase settle); multi-player party exploration (up to 4 players), trap damage distribution, Treasure Hunter (+1..+2 bonus chests), map scouting (@ちず) with stacking vision expansion (Thief/Ninja/Geomancer/Ranger + scope_goggles up to 7x7 grid) (#483, #597), and reward item finalization with inventory capacity enforcement and depot overflow routing via `deliverRewardItems` (#657).
- **Battle Replays** (`internal/replay`): ✅ Turn-log recorder + history viewer (keyset cursor).
- **Endurance Challenge** (`internal/challenge`): ✅ 4-tier survival; Valkey Master session buffer; multi-player party challenge runs (up to 4 players, legacy HP carryover), Hall of Fame records with gravestones (`chr/099.gif`) for fallen members (#483, #600), and cashout reward finalization with `progression.ApplyExperience` level-up evaluation, `id.New()` ULID generation, and depot overflow routing via `deliverRewardItems` (#657).
