...
- **Item, Inventory, Equipment** (`internal/core/item`, `internal/inventory`, `internal/equipment`): 5カテゴリJSONカタログ（武器・防具・盾・アクセ・消費/素材）、原典Perl CGI（`lib/_data.cgi` の `@ites` 全269件）に100%パリティ適合するアイテム使用ロケーションカテゴリ（`UsageCategory`: 0=なし 43件、1=戦闘中のみ 53件、2=いつでも 53件、3=戦闘中自動 117件、4=クエスト参加時 3件）の型安全な導入・全件検証、自宅アイテム使用（`UseHomeItem`）での戦闘専用アイテムの正当な拒否（`ErrCannotUseHere` / `"%sは戦闘中でしか使えません"`）、戦闘コマンド（`ActionItem`）における非戦闘アイテム拒否と使用場所検証（`ValidateUsageLocation` / `ErrCannotUseInCombat`）、スロット装備、所持枠管理、統一アイテム定義プロバイダー（`coreitem.DefinitionProvider`）、インベントリアイテム更新（`inv.Update`）、装備スロットカプセル化（`equip.Equip`, `equip.Unequip`）。
- **Battle** (`internal/core/battle`): 決定論的ターン制戦闘解決、勝敗・報酬決定（経験値・ゴールド・アイテム・ちいさなメダル）、構造化ターンログ出力、戦闘参加者（Participant）標準アダプタ/ビルダー（`NewParticipantFromCharacter`, `ParticipantBuilder`）。従来の1v1戦闘（`Resolve`）との100%下位互換を維持しつつ、オリジナルParty2 CGI（`_battle.cgi`, `_skill.cgi`）完全パリティのマルチターン・パーティ戦闘エンジン（`ResolvePartyBattle`）を実装（Issue #480）。最大4人の味方パーティ vs 1〜N体の敵グループ、敏捷性（Agility）降順ターン解決、MP消費の職業スキル（単体/全体・属性・回復）、CMP消費・詠唱セリフ出力付き合成スキル（GemEffect連鎖）、戦闘アイテムコマンド（`ActionItem` / `@どうぐ`）の実行および `UsageCategoryCombatOnly` 厳格バリデーション（`ValidateItemAction` / `ErrCannotUseInCombat` による種・メダル・素材・パッシブ等の不正使用拒否、全269アイテムマトリクステスト完備）、属性フィールド状態（火・水・風・土・光・闇の相性補正 +30%/-20%・反属性フィールド無効化・ターン減算）、および致命傷時の復活トリガー（ファラオ、不死身、闘気の盾、ドクロのお守り、呪いの復活）の完全再現を達成。ファイルサイズ上限厳守のため単一責務ファイル（`party_battle.go`, `execute_item.go`, `field.go`, `defeat.go`, `action.go`）に分割完了。
...
- **Database & Transaction Orchestration** (`internal/database`, `internal/economy`, `internal/core/event`, `internal/testutil`, `internal/database/testutil`): 全リポジトリのトランザクション伝播モデル（`RunInTx` と `ExecutorFromContext`）、コンテキスト内トランザクション再利用、決定論的行ロック獲得順序（Shared -> Players -> Characters (昇順) -> Inventory/Equipment -> Jobs -> Depots -> Guilds (昇順) -> 各種機能テーブル）のAST強制（`internal/database/lock_hierarchy_lint_test.go`）、共通2者間IDソート排他ロックユーティリティ（`id.Sort2`）、**横断的アプリケーション実行時プリミティブ層**（`docs/architecture/cross-domain-primitives.md`、`economy.TransactionRunner`、`economy.ExecuteTransaction`、`economy.Run[T]`、機能パッケージでの直接 `database.RunInTx` 呼び出し禁止のAST静的解析機械的検証 `internal/architecture/tx_runner_lint_test.go`、`internal/home` の `BuildHouse`/`UseHomeItem` および `internal/job` の `RecallFutureMemory` 等の複数リソース変更操作への完全適用）、インプロセス2フェーズドメインイベントディスパッチャ（`internal/core/event.Dispatcher`、In-Tx同期＋Post-Commit非同期）、標準エンティティファクトリおよび汎用並行ストレステストハーネス（`RunConcurrentStressTest`, `RunRace`, `RunRace2`）。`internal/economy/economy.go` は責務別に4ファイルへ分割リファクタリングされ500行制限をクリア（`whitelistedLegacyFileLimits` から完全除外）。
...
- **Bank** (`internal/bank`): 銀行（窓口NPC「タクシード」`@タクシード`、キャラクター別ゴールド預託、預金・引出・999,999G財布上限クランプ＆超過分預金返還、最大預金額99兆9999億9999万9999G、手数料0、`characters.deposit` への移行、架空のプレイヤー間直接送金および架空テーブル `bank_accounts`/`bank_transfers` の完全撤廃、`characters` 行ロックによるアトミック整合性担保、Issue #476）。
...
- **Player Private Home, Town Estates & Resting** (`internal/home`, `internal/town`, `internal/core/timer`): 町での家建築・維持サイクル管理（メケメケ村 500G/5日、キノコ町 1500G/10日、スライム町 3000G/15日、ガイア国 5000G/20日、全町各定員10軒、1キャラ1軒制約、スタイル001〜028バリデーション、`POST /towns/{town_id}/houses`、`GET /towns/{town_id}/houses`、`GET /houses/check?target=...`、MariaDB `expires_at` および Valkey タイマーリース `party2:timer:house:<character_id>`）、自宅からの倉庫・手持ちアイテム利用（`GET /characters/{id}/home/items`、`POST /characters/{id}/home/items/use`、武器・防具の強さ・重さ・価格鑑定、ステータス種・幸せの種・ファイト一発（気合の霊薬）・小さなメダルの使用、`economy.TransactionRunner` を介した決定論的行ロック階層（Rank 2 `characters` → Rank 3 `inventory_items` / Rank 5 `character_depots`）によるアトミック消費・ステータス更新・障害時完全ロールバック保証、高並行実行時の競合・ロストアップデート・デッドロック防止検証、OverLevel時のステータス種上昇クランプ、戦闘専用回復アイテム（薬草・聖水等）の自宅使用禁止 400 Bad Request）、プレイヤー文字色設定（`characters.color` / `POST /characters/{id}/color`）、ペット言葉学習の原典旧仕様是正（最大30フレーズ、1フレーズ最大120文字）、不要な創作カラム・機能（壁紙テーマ、モットー、訪問者カウンター）の完全撤廃、手紙送受信・独立削除フラグ・未読件数管理、差出人・受取人並行削除時の競合・ロストアップデート防止トランザクション保護（`DeleteLetter` の `RunInTx` + `SELECT ... FOR UPDATE` 化と確実な物理パージ保証）、および原典準拠の無料睡眠・休息回復機能（同時接続数スケーリングカウントダウン（60s/120s/180s）、他プレイヤー私有地訪問宿泊時の有効な家保有検証（未建築・期限切れ時は 404 Not Found 拒絶）、Valkey Native TTL `party2:timer:sleep:<id>` による行動抑止 409 Conflict、起床時のHP/MP/疲労度全快、一時職記憶復元、酒場満腹度リセット、礼拝堂祈り・加護リセット連携）。リポジトリおよびサービスのISP分割（Estate, Mailbox, Companion, DeliveryNotice）およびオプション抽出により500行制限クリア。
...
- **Secret Underground Shop & NPC @ヒミツジ** (`internal/secretshop`): 秘密の店（原典 `secret.cgi` / `item.cgi:himitsunomise` 準拠の転職7回以上 `JobLevel >= 7` 入場資格判定、原典道具8種（薬草の根っこ、魔法の鏡、守りのルビー、銀のたてごと、へんげの杖、賢者の悟り、精霊の守り、伯爵の血）定価3倍価格カタログ `secret_items.json`、お助け依頼除外フィルター、手持ち満杯・複数個購入時の預かり所（Depot）自動転送、アトミック購入トランザクション、NPC会話・詳細情報・ぱふぱふサービス（回復なし・会話のみの原典回帰））。
...
- **Town Black Market & Underworld Barter @闇商人** (`internal/blackmarket`): 闇市場（原典 `lib/black_market.cgi` 準拠の無条件入場、創作のGold売買・相場変動・日次制限枠・創作テーブルを完全撤廃、手持ちおよび預かり所（Depot）からのレアアイテム生贄 `SacrificeItem`（46種原典アイテム・レアポイント/Uレアポイント加算）、原典24種限定景品（通常12種・裏12種）のポイント交換 `TradePrize`、満杯時容量検証および景品の預かり所（Depot）直送、原典台詞・詳細情報会話、決定論的行ロック階層（Rank 2 `characters` → Rank 3 `inventory_items` → Rank 5 `character_depots` → Rank 8 `blackmarket_character_points`）による完全アトミック整合性）。
...
- **Flea Market & Player Item Stalls** (`internal/fleamarket`): フリーマーケット＆露店取引（原典 `free.cgi`/`depot.cgi` 準拠の預かり所（Depot）連動出品・購入時Depot直送・キャンセル時Depot安全返却、サーバー全体120件上限制約（`ServerMaxListings = 120`）、OverFlea連動出品枠（最大5〜10件）、1〜999,999 G固定価格出品、決定論的行ロック階層（Rank 0 `fleamarket_listings` → Rank 2 `characters` 昇順 → Rank 5 `character_depots`）によるデッドロック防止、SQL CAS述語 `WHERE id = ? AND status = 'active'` と `RowsAffected() == 1` 検証によるアトミック移転）。
- **Gem Store, Dedicated Gem Box Storage & Appraisal** (`internal/gemstore`): 宝石店・専用宝珠箱・宝珠/天珠販売・特殊合成加工・他プレイヤー譲渡・未鑑定宝珠鑑定（旧原典 `gem_box.cgi` / `system.cgi` 準拠の専用ストレージ `character_gem_boxes` / `gem_box_items`、転職回数 `job_lv` 連動の動的容量計算 `job_lv >= 20 ? 100 : job_lv * 5 + 5`、満杯時拒否バリデーション、カタログ順整頓 `POST /characters/{id}/gembox/sort`、レベル別カタログ `gems.json`、55種以上の上位合成レシピ `recipes.json`、5種未鑑定宝珠の重み付きランダム鑑定プール `orb_appraisals.json`、決定論的行ロック階層（Rank 2 `characters` → Rank 3 `inventory_items` → Rank 5 `character_depots` → Rank 8 `character_gem_boxes`）によるアトミック整合性）。
...
- **Player Store, Town Boutiques & Interiors** (`internal/store`): プレイヤーストア・町建築バザール・インテリア（原典 `store.cgi` 準拠の店舗建築（50,000 G、各町定員10店舗、90日間維持サイクル、ギルド所属時900 GP獲得）、預かり所（Depot）からのゴールド販売出品（1〜9,999,999 G）および物々交換出品、OverStore連動出品枠（10〜20枠）、購入・物々交換アトミックトランザクション、看板名変更（5,000 G・最大8文字）、26種壁紙変更（0〜10,500 G）、15種家具インテリア配置（1,000 G・最大5個）、家具命名（最大8文字）、全家具撤去、決定論的行ロック階層（Rank 0 `store_sales` → Rank 2 `characters` 昇順 → Rank 5 `character_depots` 昇順）遵守）。
