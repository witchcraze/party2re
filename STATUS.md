...
Last updated: Issue #369 — [Architecture] Core/RunState: Evaluate Valkey Master for in-progress transient run sessions (Dungeon & Challenge)
...
- **Database**: MariaDB（マイグレーション `migrations/001_initial.sql` 〜 `052_drop_parties_and_party_members.sql`、`make db-migrate` / `make db-reset`、プレイヤー資産・キャラクター・所持品・経済の永続権威 MariaDB Master、揮発性 `player_sessions`, `parties`, `party_members` テーブルの完全廃止）。
