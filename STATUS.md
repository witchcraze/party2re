...
- **Valkey**: 遅延アクションキュー・排他ロック・分散レートリミット・ランキングスナップショットキャッシュ（AOF+RDB永続化）。RFC #356 に基づく揮発性ステートのプライマリストア（Valkey Master）境界策定、システムメンテナンス状態マスター移行（Issue #367）およびプレイヤー認証セッションマスター移行（Issue #366、7日間TTLネイティブ失効、アカウント削除連動パージ）完了。待機ロビー・一時ランバッファ・ボスHPの段階的移行ロードマップ確立（Issue #368, #369, #370）。
