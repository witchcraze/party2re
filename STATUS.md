...
Last updated: Issue #379 — [Feature] Party/Medal: Connect party adventure victories and rewards to achievement progress tracking
...
- **Valkey Keyspace Taxonomy & Operational SSOT (`docs/architecture/valkey-keyspace.md`)**: システム全体の Valkey キー空間（`party2:<namespace>:<entity>[:<id>]`）、データ型、TTLポリシー、所有モジュール、逆引き Set インデックスパターンを SSOT ドキュメントとして定義。Go AST リンター（`internal/architecture/valkey_lint_test.go`）により本番コードでの `KEYS *` 呼び出し禁止、登録外プレフィックスの排除、ドキュメント同期を機械的に検証（高速バイト事前フィルタにより 191 ファイル検査 0.01 秒、`make check` step `[4/7]` 統合）。
