...
Last updated: Issue #385 — [Architecture/Testing] CI: Standardize wall-clock test budget policies and establish comprehensive continuous benchmark verification framework
...
- **Automated Mechanical Verification**: Go AST シンボルリント（`internal/architecture/arch_test.go`）による 0.04 秒の静的シンボル実在性チェック（モジュール定義および共有テーブル逆引き定義、ASTキャッシュおよびバイト事前検査）およびトランザクション境界シンボルにおける `RunInTx` 呼び出しの実在検証（`go test ./...` および `scripts/verify.sh` step `[4/7]` 統合）。外部ランタイム不要の純粋な Go 標準構文解析器による機械的テスト。
- **Continuous Performance Verification & Benchmark Framework (`docs/development/benchmarking.md`)**: CI/仮想化環境におけるユニットテスト実行時間アサーション（1s未満閾値）の不安定化（flakiness）を根本排除し、デッドロック検知用の安全上限（2〜5s）と `t.Logf` への統一方針を `.agents/rules/01-development-workflow.md` に明文化。代わりにクリティカルパス（AST静的解析リント `BenchmarkLockHierarchyLinter`, `BenchmarkLockEvaluationStatement`, `BenchmarkGuidanceLayerLinter`, `BenchmarkValkeyLinter`, `BenchmarkCoreDomainInvariantLinter`、戦闘シミュレーション `BenchmarkBattleSimulation`, `BenchmarkBattleSimulationMultiTurn`、Valkeyセッションリポジトリ操作 `BenchmarkValkeySessionSave`, `BenchmarkValkeySessionFind`）を網羅する `Benchmark*` スイート、標準実行スクリプト（`scripts/benchmark.sh`）、`Makefile` ターゲット（`make bench`）、およびベースライン比較・リグレッション自動検知CLI（`scripts/compare_benchmarks.go`）を配備。
- **Valkey Keyspace Taxonomy & Operational SSOT (`docs/architecture/valkey-keyspace.md`)**: システム全体の Valkey キー空間（`party2:<namespace>:<entity>[:<id>]`）、データ型、TTLポリシー、所有モジュール、逆引き Set インデックスパターン、Lua スクリプト運用基準（1ms 未満バジェット、計算量上限 O(log N)、Lua 内 `KEYS *` 禁止、Cluster Hash Tagging `{...}` 規約、インメモリフォールバック等価性、スクリプトレジストリ）、および TTL スコア付き Sorted Set（ZSET）による遅延パージ運用標準（`ZREMRANGEBYSCORE` によるクエリ時パージ、`WRONGTYPE` ゼロダウンタイムマイグレーション、Candidate D 探索バッファ等への設計ブループリント）を SSOT ドキュメントとして定義（`.agents/rules/05-database-and-caching.md` Section 3.5/3.6, `docs/architecture/valkey-keyspace.md` Section 2.3/5.5/5.6/5.7）。Go AST リンター（`internal/architecture/valkey_lint_test.go`）により本番コードでの `KEYS *` 呼び出し禁止、登録外プレフィックスの排除、Lua スクリプト内禁止コマンドの排除、ZSET遅延パージ標準とドキュメント同期を機械的に検証（高速バイト事前フィルタにより検査 0.01 秒、`make check` step `[4/7]` 統合）。

- **Core Domain Invariant Static Analysis Linter Suite**: Go AST 静的構文解析リンター（`internal/core/core_lint_test.go`）による 186+ 生産コードファイルの網羅的検査（バイト事前フィルタ適用により 0.05 秒）。Progression（`Experience`, `Level`）、Currency & Economy（`Money`, `SmallMedals`）、Job State（`CurrentJobID`, `MasteredJobs`）、Inventory（`Inventory.Items`）、Equipment（`Equipment.Slots`）、Battle Participant Identity（`Participant.ID` への `.Name` 指定禁止）の全6重要ドメイン不変条件に対する直接構造体フィールド操作を機械的に禁止し、Core標準カプセル化ヘルパー経由の操作（`progression.ApplyExperience`, `c.AddMoney`, `c.DeductMoney`, `c.AddSmallMedals`, `c.DeductSmallMedals`, `inv.Add`, `inv.Consume`, `inv.Update`, `equip.Equip`, `equip.Unequip`, `battle.NewParticipantFromCharacter`）を100%強制。

### Core & Shared Components
- **Player** (`internal/core/player`, `internal/player`): アカウント登録・パスワードハッシュ・セッション管理（Valkey Master `party2:session:<token>` によるネイティブ7日間TTL自動失効・リクエスト毎のSQL接続完全排除、`party2:player:sessions:<player_id>` を Sorted Set (`ZSET`) で追跡し有効期限スコアによる遅延パージ `ZREMRANGEBYSCORE` で期限切れトークン蓄積およびインメモリリークを完全解消 `internal/player/valkey_session_repository.go`）・Personal Access Token（APIキー `p2_sk_...`、32バイト暗号乱数エントロピー生成、MariaDB `player_api_tokens` へのSHA-256ダイジェスト永続化、平文非保持・漏洩防止、`Authorization: Bearer <token>` プレフィックス判別による透過的デュアル認証、最終利用日時更新、所有権検証付き失効 `DELETE /player/tokens/{id}`）・アカウント完全削除（`DELETE /players/me`, `DELETE /players/{id}`、パスワード再認証、所有キャラクター全件のクリーンアップフック実行、Valkeyアクティブセッション完全破棄、PATカスケード削除、および35+テーブル連鎖削除、MariaDBトランザクション整合性）。
...
- **Database & Transaction Orchestration** (`internal/database`, `internal/economy`, `internal/testutil`, `internal/database/testutil`): 全32リポジトリのトランザクション伝播モデル（`RunInTx` と `ExecutorFromContext`）への完全統一、トランザクション境界外からの直接 `r.db.BeginTx` 呼び出しの完全排除、コンテキスト内トランザクション再利用、マルチモジュール統合テスト（コミット原子性・ロールバック整合性・ネストトランザクション伝播検証）、決定論的行ロック獲得順序（Shared -> Players -> Characters (昇順) -> Inventory/Equipment -> Jobs -> Depots -> Bank -> Guilds (昇順) -> 各種機能テーブル）によるデッドロック防止の標準化および Go AST 静的解析リンター（`internal/database/lock_hierarchy_lint_test.go`）による機械的強制、共通2者間IDソート排他ロックユーティリティ（`id.Sort2`）、および再利用可能なトランザクション内経済交換ヘルパー（`internal/economy`、ゴールド・メダル増減・送金、インベントリ消費・付与、複合 `Exchange`、オーバーフロー安全乗算 `SafeMultiply`）。さらに、ドメイン制約（名前長 <= 32 文字、`id.New()[:8]` 一意サフィックス自動付与による衝突防止、パスワードハッシュ）を自動強制する標準エンティティファクトリ（`CreateTestPlayer`, `CreateTestCharacterWithFunds`, `CreateTestGuildWithLeader`, `CreateTestInventoryWithItems`, `CreateTestDepot`）および開始バリア同期・アトミックメトリクス収集・デッドロック自動検知・環境適応型負荷スケーリングを備えた汎用並行ストレステストハーネス（`RunConcurrentStressTest`, `RunRace`, `RunRace2`）を中央集約し、P2P・共有状態変更機能に対する並行ストレステストを DoD および規約として義務化。
...
- **Casino** (`internal/casino`): カジノコイン両替（1 Coin = 20 G）、インディアンポーカー（52枚標準トランプモデル、ブラインド賭け、NPCディーラーAI、最大5ラウンド・レート上昇、ショーダウン勝敗判定・配当精算、MariaDB `casino_poker_sessions` によるマルチラウンドセッション永続化、進行中カードマスキング `ClientView`、コール/勝負/降りアクションAPI、事前残高検証とアトミックベット減算による残高ちょうどでのベット誤判定防止、キャラクター親テーブル `casino_accounts` 先行行ロックによる並行性・デッドロック保護）、スロットマシン（3リール・5絵柄、777 100倍ジャックポット、レート設定 $1〜$200）、ドッペルゲンガー（8種マーク一致・秘密選択、4x/6x/8x 倍率設定）、ハイロー（トランプ数字大小予測、2倍配当、連勝継続倍々モード）。全ゲームにおいて `DeductBetAndCreditPayout` による条件付きアトミックベット減算・配当付与トランザクション処理と MariaDB 行レベルロックによって並行性エクスプロイト（残高0での無限無料スピン・不正配当獲得）を完全防止。
...
- **Battle Replays & Match History** (`internal/replay`): 戦闘リプレイ・対戦履歴（全戦闘モードのターン別アクションログ・ダメージ値・残りHPスナップショットの記録・忠実再生、標準化レコーダー `ReplayRecorder` / `RecordMatchFromResult` / `RecordCharacterVsCharacter` / `RecordCharacterVsMonster` / `RecordParticipantVsParticipant`、プレイヤー別対戦履歴・全体最新リプレイ一覧（キーセット・カーソルページネーション対応）、保持期間経過レコードの自動プルーニング、MariaDB永続化）。
...
- **Town Park & Public Bulletin Board** (`internal/park`): 交流広場・公開掲示板（プレイヤー発言投稿・文字色指定・宛先指定・HTMLサニタイズ・連投レートリミット、最新投稿ページネーション（キーセット・カーソル対応）、@町娘NPC会話・20種運勢＆27色ラッキーカラー占い、MariaDB永続化）。
...
- **Player Private Home & Mailbox** (`internal/home`): 自宅・私有地管理（`home.cgi`、壁紙・テーマ・一言設定、訪問者記録、プレイヤー間手紙送受信・受信箱/送信箱（オフセット/カーソル両対応）・未読件数、送信者・受信者の独立削除フラグ `is_deleted_by_sender`/`is_deleted_by_recipient` と双方削除時の完全パージ、仲間ペット言葉教え・挨拶会話、送金・譲渡通知台帳、MariaDB永続化）。
...
- **Town Delivery Quests & Player Courier Service** (`internal/delivery`): 町のでりばりー依頼＆プレイヤー間宅配便（薬草・ポーション・装備等のNPC配送依頼、最大3件同時受領、インベントリ消費・ゴールド/EXP/アイテム報酬アトミック精算、およびプレイヤー間アイテム/ゴールド宅配便、50 G手数料、受取待ち・発送履歴（キーセット・カーソル対応）、受取・発送キャンセル/返金、`GetParcelByIDForUpdate` および CAS 条件付きステータス更新 `WHERE id = ? AND status = 'pending'` による受取/キャンセル並行実行時の二重支払い・複製レースコンディション完全防止、MariaDB永続化）。
- **Flea Market & Player Item Stalls** (`internal/fleamarket`): フリーマーケット＆プレイヤー露店取引（`free.cgi`、最大5件同時出品、1〜999,999 G固定価格出品、出品時インベントリ消費・キャンセル時安全返却、`GetListingByIDForUpdate` および買い手・売り手キャラクターID昇順ソート排他ロックによる並行購入時のデッドロック完全防止とゴールド・アイテムアトミック移転、`UpdateListing` の明示的 SQL CAS 述語 `WHERE id = ? AND status = 'active'` と `RowsAffected() == 1` 検証・ステータス競合時 `ErrListingNotActive` 早期返却、50並行バイヤー・100反復同時購入競合および購入vs出品取消競合の MariaDB 実機ストレステスト `TestConcurrencyStressFleaMarketPurchaseVsCancel` による二重購入・アイテム増殖・資産消失ゼロおよびデッドロックフリー検証完了、MariaDB永続化 `fleamarket_listings`）。
- **Gem Store, Jewel Synthesis & Appraisal** (`internal/gemstore`): 宝石店・宝珠/天珠販売・特殊合成加工・他プレイヤー譲渡・未鑑定宝珠鑑定（`gem_store.cgi`, `_data.cgi` No. 251–255, NPC `@ジェマ`、レベル別カタログ `gems.json`、55種以上の上位宝石合成レシピ `recipes.json`、5種未鑑定宝珠の重み付きレガシー準拠ランダム鑑定プール `orb_appraisals.json`、スレッドセーフRNG/決定論的シード注入、50%売却、自身送信防止＆昇順ロック譲渡、`TransactionProvider` と決定論的行ロック階層（`characters` -> `inventory_items`）による完全アトミックトランザクション整合性・デッドロック防止）。
...
- **Multiplayer Party & Co-op Quests** (`internal/party`): パーティ結成・冒険中のパーティー（`quest.cgi`, `party.cgi`、最大4人編成、パーティ名バリデーション、合言葉パスワード設定、参加条件チェック（Lv上限・下限、HP下限）、単一アクティブパーティ制約、準備完了（Ready）同期、リーダー権限（キック・解散）、マルチプレイヤー協力戦闘解決（`internal/core/battle` 拡張、同名キャラクター重複時も一意な `Participant.ID` による確実な識別、生存キャラクターの被ダメージ残りHP永続化、ステージ名自動補完 `StageName`）、協力シナジーボーナス（+10%〜+30% EXP/ゴールドボーナス）、報酬分配（レベルアップ・OverLevel限界突破（Lv150）対応・ドロップ品一括付与、戦闘不能メンバーも満額報酬獲得＆HP1で生存する協力プレイQoL設計）、勝利時フック（`VictoryHook`）による生涯マイルストーン実績連携（冒険勝利数、魔物討伐数、獲得ゴールド）、冒険ログ永続化、Valkey Master による揮発性待機ロビー管理（`party2:party:lobby:<party_id>` 15分TTL自動失効・孤立行ゼロ、`party2:party:ready:<party_id>:<character_id>` 60秒Readyカウントダウン自動失効、`party2:party:character:<character_id>` 所属インデックス、`party2:party:lobbies` ZSET ロビー一覧）、アトミックLuaスクリプトによるRMWレース排除（`AddMember`, `RemoveMember`, `UpdateMemberReady`, `UpdateParty`）、厳密な定員上限ガード（`max_members` 超過時 `ErrPartyFull`）、MariaDB待機ロビーテーブル（`parties`, `party_members`）の完全廃止（マイグレーション052）、MariaDB `party_adventure_logs` への恒久冒険ログ永続化およびエラー伝播担保、高並行ストレステスト（50+ goroutines 同時参加）、スレッドセーフなインメモリフォールバックサポート）。
