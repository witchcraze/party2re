...
Last updated: Issue #380 — Architecture: Resolve persistence boundary discrepancy between Valkey Master lobby and MariaDB schema, and synchronize design documentation
...
- **Player** (`internal/core/player`, `internal/player`): アカウント登録・パスワードハッシュ・セッション管理（Valkey Master `party2:session:<token>` によるネイティブ7日間TTL自動失効・リクエスト毎のSQL接続完全排除 `internal/player/valkey_session_repository.go`）・アカウント完全削除（`DELETE /players/me`, `DELETE /players/{id}`、パスワード再認証、所有キャラクター全件のクリーンアップフック実行、Valkeyアクティブセッション完全破棄、および35+テーブル連鎖削除、MariaDBトランザクション整合性）。
