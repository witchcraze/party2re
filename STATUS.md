...
Last updated: Issue #377 — Party/Valkey: Enforce atomic mutations in Valkey lobby state and add concurrent join stress tests
...
- **Database**: MariaDB（マイグレーション `migrations/001_initial.sql` 〜 `051_drop_player_sessions.sql`、`make db-migrate` / `make db-reset`、プレイヤー資産・キャラクター・所持品・経済の永続権威 MariaDB Master、揮発性 `player_sessions` テーブルの完全廃止）。
- **Valkey**: 遅延アクションキュー・排他ロック・分散レートリミット・ランキングスナップショットキャッシュ（AOF+RDB永続化）。RFC #356 に基づく揮発性ステートのプライマリストア（Valkey Master）境界策定、システムメンテナンス状態マスター移行（Issue #367）およびプレイヤー認証セッションマスター移行（Issue #366、7日間TTLネイティブ失効、アカウント削除連動パージ）完了。統一キー空間仕様（SSOT: `docs/architecture/valkey-keyspace.md`）策定および AST 機械検証（`internal/architecture/valkey_lint_test.go`）を配備（Issue #374）。待機ロビーおよび準備完了状態のValkey Master移行（Issue #368、15分TTL自動失効・60秒Readyカウントダウン・MariaDB孤立行完全排除・永続冒険ログ連携）完了。一時ランバッファ・ボスHPの段階的移行ロードマップ推進（Issue #369, #370）。
