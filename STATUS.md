...
Last updated: Issue #163 — [Feature] Auth: Player Personal Access Token (API Key) generation and authentication
...
- **Valkey**: 遅延アクションキュー・排他ロック・分散レートリミット・ランキングスナップショットキャッシュ（AOF+RDB永続化）。RFC #356 に基づく揮発性ステートのプライマリストア（Valkey Master）境界策定、システムメンテナンス状態マスター移行（Issue #367）およびプレイヤー認証セッションマスター移行（Issue #366, #378、7日間TTLネイティブ失効、Sorted Set 遅延パージによる期限切れトークン完全回収、アカウント削除連動パージ）完了。統一キー空間仕様（SSOT: `docs/architecture/valkey-keyspace.md`）策定および AST 機械検証（`internal/architecture/valkey_lint_test.go`）を配備（Issue #374）。待機ロビーおよび準備完了状態のValkey Master移行（Issue #368, #380、15分TTL自動失効・60秒Readyカウントダウン・MariaDB `parties`/`party_members` テーブル完全廃止・永続冒険ログ連携）完了。Lua スクリプト運用基準・Hash Tagging 規約・インメモリ等価性 SSOT 策定および AST 機械的検証（Issue #387）。一時ランバッファ（ダンジョン探索・連戦サバイバル）のValkey Master移行評価・Luaスクリプト契約（`dungeon_step`, `challenge_advance_round`）・Two-Phase Settlement永続化境界策定完了（Issue #369, SSOT: `docs/architecture/transient-run-state.md`）。ワールドボスHPの段階的移行ロードマップ推進（Issue #370）。
